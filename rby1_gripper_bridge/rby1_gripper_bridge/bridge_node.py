"""ROS 2 node that exposes the RB-Y1 gripper Dynamixel bus."""
...
class GripperBridgeNode(Node):
    """Translate normalized ROS commands to the vendor Dynamixel API."""
    ...
    def __init__(self) -> None:
        ...
        mock_hardware = bool(self.get_parameter('mock_hardware').value)
        requested_device = str(self.get_parameter('device_name').value)
        if mock_hardware:
            bus = MockDynamixelBus(self._motor_ids)
            self._device_name = 'mock://rby1_gripper'
            sdk_version = 'mock'
        else:
            bus, self._device_name, sdk_version = create_sdk_bus(
                requested_device,
        ...
        if bool(self.get_parameter('auto_home').value):
            self.get_logger().warning(
                'auto_home is enabled; both grippers will traverse their full '
        ...
        mode = 'mock loopback' if mock_hardware else 'real Dynamixel'
        self.get_logger().info(
            f'Gripper bridge initialized ({mode}, device={self._device_name}, '
            f'IDs={self._motor_ids}, rby1_sdk={sdk_version}). '
            'Command order is [right, left], where 0.0=open and 1.0=closed.'
        )
        if self._driver.calibration is None:
            self.get_logger().warning(
                'Gripper is not calibrated. Call gripper/home before sending '
                'commands, or configure saved calibration.'
            )
    ...
    def _home_driver(self) -> Calibration:
        ...
        self.get_logger().info(
            'Gripper homing complete. Save these values to skip homing on '
            f'future starts: min={list(calibration.minimum_rad)}, '
            f'max={list(calibration.maximum_rad)}'
        )
        return calibration
    ...
    def _on_home(self, _request, response):
        self.get_logger().warning(
            'Homing requested; keep both grippers clear while they traverse '
            'their full range'
        )
        not_ready = Bool()
        ...
        try:
            ...
            self.get_logger().error(f'Gripper homing failed: {exc}')
        self._publish_ready(force=True)
        self._publish_diagnostics()
        return response
    ...
    def _publish_diagnostics(
        ...
        status.name = f'{namespace}/gripper_bridge' if namespace else (
            '/gripper_bridge'
        )
        status.hardware_id = self._device_name
