"""ROS 2 bridge for the physical RBY1 Dynamixel grippers."""
...
class GripperBridgeNode(Node):
    """Expose the physical dual grippers through a normalized ROS interface."""
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            ...
            f'baud={self._baud_rate}, IDs={self._motor_ids}, '
            f'rby1_sdk={self._sdk_version}). '
            'Command order is [right, left], where '
            '0.0=open and 1.0=closed.'
        )
        if self._driver.calibration is None:
            self.get_logger().warning(
                'Gripper is not calibrated. Call gripper/home before '
                'sending commands, or configure saved calibration.'
            )
    ...
    def _publish_diagnostics(
        ...
        values = {
            'driver': 'rby1_sdk.DynamixelBus',
            'device_name': self._hardware_id,
            'rby1_sdk_version': self._sdk_version,
