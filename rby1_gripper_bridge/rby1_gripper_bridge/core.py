...
class GripperError(RuntimeError):
    """Base error raised by the gripper bridge core."""
...
class GripperDriver:
    ...
    def initialize(self) -> None:
        """Open the serial bus, configure it, and verify both motors."""
        ...
        with self._lock:
            ...
            constants = [1.0] * (max(self._motor_ids) + 1)
            for motor_id, constant in zip(
                self._motor_ids, self._torque_constants
            ):
                constants[motor_id] = constant
    ...
    def home(
        ...
        *,
        ...
        """Discover both encoder endpoints by gently driving to hard stops."""
        ...
        with self._lock:
            ...
            try:
                ...
                for direction in (1.0, -1.0):
                    ...
                    while stable_count < int(stall_samples):
                        ...
                        if self._clock() >= deadline:
                            raise GripperHardwareError(
                                'homing timed out before both motors stalled'
                            )

                        self._bus.group_sync_write_send_torque([
                            (motor_id, direction * homing_torque)
                            for motor_id in self._motor_ids
                        ])
                        ...
                        if all(
                            abs(now - before) <= stall_threshold_rad
                ...
                calibration = Calibration(
                    ...
                    closed_at_minimum=(
                        bool(closed_at_minimum[0]),
                        bool(closed_at_minimum[1]),
                    ),
    ...
    def set_torque_enabled(self, enabled: bool) -> None:
        """Enable safely at the current position, or disable both motors."""
    ...
    def command(self, close_ratios: Sequence[float]) -> Pair:
        """Send one normalized dual-gripper command and remember the target."""
    ...
    def read_state(self) -> GripperState:
        """Read both motor states and convert positions to close ratios."""

        with self._lock:
            self._require_initialized_locked()
            raw = self._bus.get_motor_states(list(self._motor_ids))
            if raw is None:
                self._healthy = False
                raise GripperHardwareError(
            ...
            if any(motor_id not in states for motor_id in self._motor_ids):
                self._healthy = False
                raise GripperHardwareError(
                    'motor-state response did not contain both grippers'
                )

            ordered = [states[motor_id] for motor_id in self._motor_ids]
            positions = self._state_values(ordered, 'position')
            velocities = self._state_values(ordered, 'velocity')
            currents = self._state_values(ordered, 'current')
            temperatures = self._state_values(ordered, 'temperature')
            torque_enabled = (
                bool(getattr(ordered[0], 'torque_enable', False)),
                bool(getattr(ordered[1], 'torque_enable', False)),
            )
            if self._enabled and not all(torque_enabled):
                self._enabled = False
                self._target_close_ratios = None
                self._target_positions_rad = None
            if self._calibration is None:
    ...
    def _write_positions_locked(self, positions: Pair) -> None:
        self._bus.group_sync_write_send_position([
            (motor_id, position)
            for motor_id, position in zip(self._motor_ids, positions)
        ])

    def _read_encoders_locked(self) -> Pair:
        raw = self._bus.group_fast_sync_read_encoder(list(self._motor_ids))
        if raw is None:
            self._healthy = False
            raise GripperHardwareError('failed to read gripper encoders')
        values = {int(motor_id): float(value) for motor_id, value in raw}
        if any(motor_id not in values for motor_id in self._motor_ids):
            self._healthy = False
            raise GripperHardwareError(
                'encoder response did not contain both grippers'
            )
        pair = (
            values[self._motor_ids[0]],
            values[self._motor_ids[1]],
        )
        if not all(math.isfinite(value) for value in pair):
            self._healthy = False
    ...
    @staticmethod
    def _state_values(states: Iterable[Any], field: str) -> Pair:
        values = []
        for state in states:
            try:
