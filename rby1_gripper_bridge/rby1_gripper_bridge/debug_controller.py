...
class GripperDebugController(Node):
    """Publish commands and call bridge services from a terminal."""
    ...
    def _execute_partial(self, instruction: DebugInstruction) -> None:
        ...
        if base is None:
            raise ValueError(
                'no state received yet; use set <right> <left> first'
            )
        command = list(base)
    ...
    def _publish_command(self, values: Tuple[float, float]) -> None:
        ...
        self.get_logger().info(
            f'published close ratios [right, left]={list(values)}'
        )
