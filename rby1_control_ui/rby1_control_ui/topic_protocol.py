...
The transport intentionally uses ``std_msgs/String`` so the control UI package
can be split without adding a second interface-generation package.  Keeping all
encoding here gives a future planner one documented, versioned command schema
instead of depending on Python method calls.
"""
